...
namespace SharedProject.http
{
    ...
    public class HttpHelper
    {
        ...
        public HttpResult GetHtml(string url)
        {
            return GetHtml(new HttpItem { URL = url });
        }
        /// <summary>
        /// 根据相传入的数据，得到相应页面数据
        /// </summary>
        ...
        public HttpResult GetHtml(HttpItem item)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(item.Cookie) && !string.IsNullOrEmpty(this.Cookie))
                {
                    item.Cookie = this.Cookie;
                }
                if (this.defaultHeader != null)
                    foreach (var head in this.defaultHeader)
                        item.Header.Add(head);

                //设置默认代理
                if (!string.IsNullOrWhiteSpace(this.defaultProxy))
                    item.ProxyIp = this.defaultProxy;
                //准备参数
                SetRequest(item);
            }
            catch (Exception ex)
            ...
            //自动更新cookie
            if (!string.IsNullOrWhiteSpace(result.Cookie) && AutoCookie)
        ...
        {
            this.Cookie = HttpTool.UpdateCookie(cookie, this.Cookie);
        }
        #region GetData
    ...
    public class HttpItem
    {
        ...
        string _UserAgent = "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36";
        /// <summary>
    ...
    public static class HttpTool
    {
        ...
        public static string GetSmallCookie(string strcookie)
        {
            ...
            foreach (string item in list)
            {
                ...
                //排除Max-Age
                if (itemcookie.Contains("max-age=")) continue;

                //排除重复项
                if (cookielist.Contains(item.Trim())) continue;
                var key = item.Split('=')[0];
                var s = cookielist.Where(e => e.Split('=')[0] == item.Split('=')[0]).FirstOrDefault();
        ...
        public static string UpdateCookie(string newCookie, string oldCookie)
        {
            string result = "";
            if (string.IsNullOrWhiteSpace(newCookie))//新的是空的
            {
                return oldCookie;//返回老的
            }
            if (string.IsNullOrWhiteSpace(oldCookie))//老的是空的
            {
                return HttpTool.GetSmallCookie(newCookie);//返回新的
            }
            Dictionary<string, string> rc = new Dictionary<string, string>();
            var arr = (oldCookie + ';' + HttpTool.GetSmallCookie(newCookie)).Split(';');
        ...
        public static string GetMergeCookie(string cookie1, string cookie2)
        {
            ...
            {
                return GetSmallCookie(cookie2);//返回老的
            }
            if (string.IsNullOrWhiteSpace(cookie2))//老的是空的
            {
                return GetSmallCookie(cookie1);//返回新的
            }
            List<string> cookielist = new List<string>();//结果
            string[] list_1 = GetSmallCookie(cookie1).ToString().Split(';');
            string[] list_2 = GetSmallCookie(cookie2).ToString().Split(';');
        ...
        {
            var m = Regex.Match(Url, @"(http|https)://(\w+\.?)+");
            if (m.Success)
                return m.Value;
            else
                return "";
